...
% Création : 07/02/13
% Modifications : jan. 2015, déc. 2015
...
\begin{Exercice} {\bf (Projets et graphes d'inclusions)} \smallskip
    ...
    \begin{enumerate}
        ...
        \Sol{
        (a) marche.
        (b) ne marche pas~: en effet, si l'on déplie les {\tt \#include}, comme
        le fait le pré-processeur, on observe que le prototype
        {\tt void a\_fct(B\_type x)}
        est analysé avant la déclaration du type {\tt B\_type}, ce type
        est donc inconnu. On a le message suivant de {\tt gcc} :
        \begin{center}
        {\tt
        In file included from D.h:4:0, \\
                 from B.h:4, \\
                 from B.c:1: \\
        A.h:8:12: erreur: unknown type name ‘B\_type’ \\}
        \end{center}
        Ceci est dû au fait que seuls les symboles de fonction ou de
        variables peuvent ne pas être résolus avant l'édition des liens,
        {\bf mais pas les symboles de types}.
        (c) marche.
        (d) marche.
        (e) marche.
        (f) marche.}
        ...
        \item Maintenant, on suppose que {\tt D.h} n'inclut plus {\tt A.h}.
        Écrire un {\tt Makefile} complet pour compiler le projet.

        \Sol{ \\
        {\tt
            Projet: Main.o A.o B.o C.o D.o E.o \\
            $\longrightarrow$ gcc -o Projet Main.o A.o B.o C.o D.o E.o \\

            A.o: A.c A.h B.h C.h E.h \\
            $\longrightarrow$ gcc -c A.c \\

            B.o: B.c B.h D.h \\
            $\longrightarrow$ gcc -c B.c \\

            C.o: C.c C.h \\
            $\longrightarrow$ gcc -c C.c \\

            D.o: D.c D.h \\
            $\longrightarrow$ gcc -c D.c \\

            E.o: E.c E.h D.h \\
            $\longrightarrow$ gcc -c E.c \\
        }
        }
        \smallskip
    \end{enumerate}
