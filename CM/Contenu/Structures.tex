...
permet de \alert{déclarer un type structuré} \Code{NOM}.
\bigskip
...
\begin{frame}[fragile]
\frametitle{Affectation de variables d'un type structuré}
{\bf Règle générale}~: pour chaque définition d'un type structuré \Code{X},
...
\Sortie{invalid operands to binary == (have 'A' and 'A')}
\smallskip
...
Par exemple, les {\bf tableaux} de taille \Code{n} d'éléments d'un type
\Code{T} sont organisés en un segment contigu de \Code{sizeof(T) * n}
octets.
\medskip
