...
% Création : oct. 2013
% Modifications : aout 2014 oct. 2014, déc. 2015
...
\uncover<2->{
Cette fonction renvoie le nombre de caractères affichés. Si une erreur
a lieu, elle renvoie un entier négatif.
\medskip}
...
\uncover<4->{
Que font les entrées suivantes~?
\begin{multicols}{2}
\begin{enumerate} \footnotesize
    \item \Sortie{25\textvisiblespace W\textvisiblespace abc\textbackslash n}}
    \uncover<5->{
    \item \Sortie{25\textvisiblespace W%
        \textvisiblespace\textvisiblespace abc\textbackslash n}}
    \uncover<6->{
    \item \Sortie{25Wabc\textbackslash n}}
    \uncover<7->{
    \item \Sortie{25\textvisiblespace abc\textbackslash n}}
    \uncover<8->{
    \item \Sortie{xy\textvisiblespace W\textvisiblespace abc\textbackslash n}}
    \uncover<9->{
    \item \Sortie{25\textvisiblespace w\textvisiblespace abc\textbackslash n}
    }
\end{enumerate}
...
\uncover<2->{
Cette fonction renvoie un pointeur sur une variable de type \Code{FILE}
qui est un type structuré.
\bigskip}
...
\uncover<4->{
\Code{path} désigne le chemin relatif par rapport à l'exécutable du fichier
à ouvrir.
\smallskip

\Code{mode} désigne le mode d'ouverture désiré.}
\end{frame}
...
La fonction
\begin{center}
    \Code{int fclose(FILE *fp);}
\end{center}
permet de \alert{fermer un fichier}.
\bigskip

\uncover<2->{
Cette fonction permet de mettre à jour le fichier pointé par \Code{fp}
de sorte que toutes les modifications effectuées soient prisent en compte.
Le pointeur \Code{fp} n'est plus alors utilisable pour accéder au fichier.
\bigskip}
...
La fonction
\begin{center}
    \Code{int fprintf(FILE *stream, char *format, V\_1, ..., V\_N);}
\end{center}
...
La fonction
\begin{center}
    \Code{int fscanf(FILE *stream, char *format, V\_1, ..., V\_N);}
\end{center}
...
\begin{center}
    \Code{size\_t fwrite(void *ptr, size\_t size, size\_t nmemb, \\ FILE *stream);}
\end{center}
pour \alert{écrire} dans un fichier pointé par \Code{stream} ouvert
en \alert{mode binaire}.
\bigskip

\uncover<2->{
Cette fonction écrit les \Code{nmemb} valeurs de taille \Code{size} octets
...
On utilise la fonction
\begin{center}
    \Code{size\_t fread(void *ptr, size\_t size, size\_t nmemb, \\ FILE *stream);}
\end{center}
pour \alert{lire} dans un fichier pointé par \Code{stream} ouvert
en \alert{mode binaire}.
\bigskip

\uncover<2->{
Cette fonction lit \Code{nmemb} valeurs de taille \Code{size} octets
dans le fichier pointé par \Code{stream} et les place à l'adresse pointée
