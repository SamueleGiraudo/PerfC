...
\begin{enumerate}
    \item une expression terminée par un point-virgule~;
    \smallskip
...
\uncover<2->{
P.ex., \Code{0}, \Code{'c'}, \Code{(a + 21) <\,< 3} ne sont pas a
effet de bord.
\smallskip}

\uncover<3->{
En revanche, \Code{a = 31}, \Code{printf("abc\textbackslash n")}
...
\begin{enumerate}
    \item la notion d'expression~;
    \smallskip

    \item la notion d'instruction~;
    \smallskip

    \item la notion d'effet de bord
\end{enumerate}
\end{frame}
...
\begin{multicols}{2}
\Code{$\lbrace$} \\
\quad \Code{DECLARATIONS} \\
\quad \Code{INSTRUCTIONS} \\
\Code{$\rbrace$}
\medskip

Les parties \Code{DECLARATIONS} ou \Code{INSTRUCTIONS} peuvent être
vides. Il est possible d'{\bf imbriquer} plusieurs blocs.
\end{multicols}}
\medskip
...
\}}
\end{semiverbatim}
\uncover<3->{Ceci est un bloc constitué d'une partie déclarations
et d'une partie instructions.
\bigskip

Cette dernière partie contient
un bloc constitué uniquement d'une partie instructions.
\bigskip
\bigskip
\bigskip}
\end{multicols}
...
bloc \Code{BLOC} d'une structure \Code{while}, \Code{do while} ou \Code{for}.
L'exécution continue alors à l'évaluation de l'expression test.
\medskip}

\uncover<4->{
{\bf Note}~: dans une boucle \Code{for}, l'instruction \Code{continue}
fait que l'expression d'incrémentation est tout de même évaluée.
\smallskip

Ce n'est pas le cas pour le \Code{break}.}
\begin{multicols}{2}
...
\begin{enumerate}
    ...
    \item les instructions \Code{if}, \Code{switch}, \Code{while},
    \Code{do while}, \Code{for}~;
    \smallskip

    \item les instructions de court-circuit.
\end{enumerate}
\end{frame}
...
\begin{tikzpicture}
    ...
    \uncover<5->{
    \node(adr)at(-1.5,0){\small Adresse};
...
\begin{enumerate}
    \item la notion de fonction et ses constituants~;
    \smallskip

    \item la différence entre déclaration et définition~;
    \smallskip

    \item la différence entre paramètres et arguments~;
    \smallskip
    
    \item les variables locales à une fonction~;
    \smallskip

    \item la pile~;
    \smallskip
    
    \item la notion d'effet de bord pour une fonction.
\end{enumerate}
\end{frame}
...
\uncover<4->{
Habituellement, les inclusions sont réalisés au début du programme.}
\end{frame}
