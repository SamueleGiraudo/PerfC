...
% Création : fév. 2014, jan 2015, mars 2015, déc. 2015
...
\begin{enumerate}
    ...
    (\Code{\#define}, \Code{\#ifndef}, {\em etc.}).
\end{enumerate}
\medskip
...
\scalebox{.8}{\begin{tikzpicture}
    ...
    \draw[Fleche](Main)edge[bend right=20]node{}(A);
    \draw[Fleche](Main)--(B);
    \draw[Fleche](Main)edge[bend left=20]node{}(C);
    \draw[Fleche](B)--(A);
...
Si on modifie par la suite le module \Code{A}, il suffit d'exécuter les commandes
\begin{multicols}{2}
    \Code{gcc -c A.c} \\
    \Code{gcc Main.o A.o B.o C.o} \\
...
Il est inutile de recompiler \Code{B}, \Code{C} et \Code{Main}
...
\begin{enumerate}[(1)]
    ...
    \begin{enumerate}[(a)] \normalsize
        \item si \Code{A.c} ou \Code{A.h} ont été modifiés {\bf après}
        \Code{A.o} ou bien si \Code{A.o} n'existe pas, (re)compiler le
        module \Code{A}~;
    \end{enumerate}
...
d'en-tête des modules dont dépend \Code{A} qui, s'ils sont modifiés,
doivent entraîner une recompilation de \Code{A}.})
\end{frame}
...
\scalebox{.8}{\begin{tikzpicture}
    ...
    \node[Sommet](Main)at(2,-1){\Code{Main}};
    \draw[Fleche](Main)edge[bend right=20]node{}(A);
    \draw[Fleche](Main)edge[bend left=20]node{}(B);
\end{tikzpicture}}
\end{center}
\bigskip
\bigskip
...
Supposons que l'on dispose de deux modules \Code{A} et \Code{B} tels que
\begin{center}
\scalebox{.8}{\begin{tikzpicture}
    \node[Sommet](A)at(0,0){\Code{A}};
...
Ceci signifie qu'il y a un \Code{\#include "A.h"} dans le fichier
\Code{B.h}.
\smallskip

Supposons qu'un type \Code{T} ait été déclaré dans \Code{A} et que \Code{B}
utilise ce type.
\bigskip
...
\scalebox{.4}{\begin{tikzpicture}[scale=.8]
    \node[Sommet,minimum size=2cm,font=\Large](Piece)at(-4,0){\Code{Piece}};
    \node[Sommet,minimum size=2cm,font=\Large](Case)at(-4,-4){\Code{Case}};
    \node[Sommet,minimum size=2cm,font=\Large](Position)at(0,-2){\Code{Position}};
    \node[Sommet,minimum size=2cm,font=\Large](IA)at(4,0){\Code{IA}};
    \node[Sommet,minimum size=2cm,font=\Large](IGraph)at(4,-4){\Code{IGraph}};
    \node[Sommet,minimum size=2cm,draw=Vert,fill=Vert!20,font=\Large](Main)at(7,-2){\Code{Main}};
...
est la suite des noms des fichiers \Code{.h} dont le module \Code{Ak} dépend.
\end{frame}
...
\begin{center}
    \begin{tikzpicture}[every text node part/.style={align=left},yscale=.9]
        ...
        \node[font=\scriptsize \tt](edliens)at(7,-1){gcc Main.o A.o B.o -lm -lMLV};
        \draw(A)edge[->,anchor=south,font=\scriptsize \tt] node{gcc -c A.c}(Ao);
        \draw(B)edge[->,anchor=south,font=\scriptsize \tt] node{gcc -c B.c}(Bo);
        \draw(Main)edge[->,anchor=south,font=\scriptsize \tt] node{gcc -c Main.c}(Maino);
